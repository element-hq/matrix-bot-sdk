...
import { doHttpRequest, DoHttpRequestOpts, RawResponse, ResponseBody } from "./http";
...
export class MatrixClient extends EventEmitter {
     ...
     */
    @timedMatrixClientFunctionCall()
    public async uploadContentFromUrl(url: string): Promise<string> {
        const requestId = ++this.requestId;
        const response = await getRequestFn()(url, { method: "GET" }).catch((err) => {
        ...
        return await this.uploadContent(Buffer.from(await response.body.bytes()), contentType);
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public doRequest<IsRaw extends boolean = false, NoEncoding extends boolean = false>(
        method: "GET" | "POST" | "PUT" | "DELETE",
        endpoint: string,
        qs: ParsedUrlQueryInput | null = null,
        body: any | null = null,
        timeout = 60000,
        raw?: IsRaw,
        ...
        if (this.impersonatedDeviceId) {
            // org.matrix.msc3202.device_id set until we're happy to lose compatibility with Synapse older than v1.141
            if (!qs) {
                qs = {
                    "device_id": this.impersonatedDeviceId,
                    "org.matrix.msc3202.device_id": this.impersonatedDeviceId
                };
            }
            else {
                qs["device_id"] = this.impersonatedDeviceId;
