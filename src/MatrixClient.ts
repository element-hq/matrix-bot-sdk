...
import {
  ...
  IKeyBackupInfoUpdate,
  IKeyBackupVersion,
  KeyBackupVersion,
} from "./models/KeyBackup";
...
export class MatrixClient extends EventEmitter {
  ...
  constructor(
    ...
    if (this.cryptoStore) {
      if (!this.storage || this.storage instanceof MemoryStorageProvider) {
        LogService.warn(
          "MatrixClientLite",
          "Starting an encryption-capable client with a memory store is not considered a good idea.",
        );
      }
      if (!(this.cryptoStore instanceof RustSdkCryptoStorageProvider)) {
        throw new Error(
          "Cannot support custom encryption stores: Use a RustSdkCryptoStorageProvider",
        );
      }
  ...
  public async getCapabilities(): Promise<MatrixCapabilities> {
    if (
      !this.cachedCapabilites ||
      Date.now() - this.capabilitesLastFetched >= CAPABILITES_CACHE_MS
    ) {
      this.cachedCapabilites = (
        await this.doRequest("GET", "/_matrix/client/v3/capabilities")
      ).capabilities;
  ...
  public async getSafeAccountData<T>(eventType: string, defaultContent: T = null): Promise<T> {
    try {
      ...
      LogService.warn(
        "MatrixClient",
        `Error getting ${eventType} account data:`,
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getSafeRoomAccountData<T>(
    eventType: string,
    roomId: string,
    defaultContent: T = null,
  ): Promise<T> {
    try {
      ...
      LogService.warn(
        "MatrixClient",
        `Error getting ${eventType} room account data in ${roomId}:`,
        extractRequestError(e),
      );
      return defaultContent;
    }
   ...
   */
  @timedMatrixClientFunctionCall()
  public async setPresenceStatus(
    presence: PresenceState,
    statusMessage: string | undefined = undefined,
  ): Promise<any> {
    return this.doRequest(
      "PUT",
      "/_matrix/client/v3/presence/" + encodeURIComponent(await this.getUserId()) + "/status",
      null,
      {
        presence: presence,
  ...
  public getDirectoryVisibility(roomId: string): Promise<"public" | "private"> {
    roomId = encodeURIComponent(roomId);
    return this.doRequest("GET", "/_matrix/client/v3/directory/list/room/" + roomId).then(
      (response) => {
  ...
  protected async startSync(
    emitFn: (emitEventType: string, ...payload: any[]) => Promise<any> = null,
  ) {
    // noinspection ES6RedundantAwait
    ...
    const promiseWhile = async () => {
      ...
      try {
        ...
        const backoffTime =
          SYNC_BACKOFF_MIN_MS + Math.random() * (SYNC_BACKOFF_MAX_MS - SYNC_BACKOFF_MIN_MS);
      ...
      for (let event of room["timeline"]["events"]) {
        if (event["type"] === "m.room.member" && event["state_key"] === (await this.getUserId())) {
          if (
            ...
            this.lastJoinedRoomIds.indexOf(roomId) === -1
          ) {
  ...
  public async getEvent(roomId: string, eventId: string): Promise<RoomEvent<IJsonType>> {
    ...
    if (event["type"] === "m.room.encrypted" && (await this.crypto?.isRoomEncrypted(roomId))) {
      return this.processEvent(
        (await this.crypto.decryptRoomEvent(new EncryptedRoomEvent(event), roomId)).raw,
      );
    }
  ...
  public getRoomStateEvent(roomId: string, type: string, stateKey: string): Promise<any> {
    ...
    return this.doRequest("GET", path).then((ev) => this.processEvent(ev));
  }
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getRoomStateEventContent(
    roomId: string,
    type: string,
    stateKey: string,
  ): Promise<APIRoomStateEvent["content"]> {
    const path =
      "/_matrix/client/v3/rooms/" +
      encodeURIComponent(roomId) +
      "/state/" +
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getRoomStateEventBody(
    roomId: string,
    type: string,
    stateKey: string,
  ): Promise<APIRoomStateEvent> {
    ...
    const eventRaw = (await this.getRoomState(roomId)).find(
      (ev) => ev.type === type && ev.state_key === stateKey,
    );
    if (!eventRaw) {
      ...
      throw new MatrixError(
        {
          errcode: "M_NOT_FOUND",
          error: "Could not find state event in full room state (SDK Error).",
        },
        404,
        {},
      );
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getEventNearestToTimestamp(
    roomId: string,
    ts: number,
    dir: "f" | "b",
  ): Promise<{ event_id: string; origin_server_ts: number }> {
  ...
  public async joinRoom(roomIdOrAlias: string, viaServers: string[] = []): Promise<string> {
    const apiCall = (targetIdOrAlias: string) => {
      ...
      return this.doRequest("POST", "/_matrix/client/v3/join/" + targetIdOrAlias, qs, {}).then(
        (response) => {
  ...
  public getJoinedRooms(): Promise<string[]> {
    return this.doRequest("GET", "/_matrix/client/v3/joined_rooms").then(
      (response) => response["joined_rooms"],
    );
  }
  ...
  public async getJoinedRoomMembersWithProfiles(
    roomId: string,
  ): Promise<{ [userId: string]: { display_name?: string; avatar_url?: string } }> {
    return (
      await this.doRequest(
        "GET",
        "/_matrix/client/v3/rooms/" + encodeURIComponent(roomId) + "/joined_members",
   ...
   */
  @timedMatrixClientFunctionCall()
  public getRoomMembersByMembership(
    roomId: string,
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getRoomMembersWithoutMembership(
    roomId: string,
  ...
  public createRoom(properties: RoomCreateOptions = {}): Promise<string> {
    return this.doRequest("POST", "/_matrix/client/v3/createRoom", null, properties).then(
      (response) => {
        return response["room_id"];
      },
    );
   ...
   */
  @timedMatrixClientFunctionCall()
  public async userHasPowerLevelFor(
    userId: string,
    roomId: string,
    eventType: string,
    isState: boolean,
   ...
   */
  @timedMatrixClientFunctionCall()
  public async userHasPowerLevelForAction(
    userId: string,
    roomId: string,
   ...
   */
  @timedMatrixClientFunctionCall()
  public async calculatePowerLevelChangeBoundsOn(
    targetUserId: string,
    roomId: string,
  ): Promise<PowerLevelBounds> {
    const myUserId = await this.getUserId();

    const canChangePower = await this.userHasPowerLevelFor(
      myUserId,
      roomId,
      "m.room.power_levels",
      true,
   ...
   */
  public async mxcToHttpThumbnail(
    mxc: string,
    width: number,
    height: number,
    method: "crop" | "scale",
    ...
    return (
      downloadUri.replace("/download", "/thumbnail") +
    ...
    return new Promise<{ body: Buffer; contentType: string }>((resolve, reject) => {
      ...
      getRequestFn()(params, (err, response, resBody) => {
        if (err) {
          ...
          LogService.trace(
            "MatrixClientLite",
            "(REQ-" + requestId + " RESP-H" + response.statusCode + ")",
            "<data>",
          );
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getRoomUpgradeHistory(
    roomId: string,
  ): Promise<{ previous: RoomReference[]; newer: RoomReference[]; current: RoomReference }> {
      ...
      try {
        ...
        if (createEvent["predecessor"] && createEvent["predecessor"]["room_id"]) {
          ...
          try {
            ...
            const tombstone = roomState.find(
              (e) => e["type"] === "m.room.tombstone" && e["state_key"] === "",
            );
            const create = roomState.find(
              (e) => e["type"] === "m.room.create" && e["state_key"] === "",
            );
  ...
  public async checkOneTimeKeyCounts(): Promise<OTKCounts> {
    return this.doRequest("POST", "/_matrix/client/v3/keys/upload", null, {}).then(
      (r) => r["one_time_key_counts"],
    );
  }
  ...
  @timedMatrixClientFunctionCall()
  @requiresCrypto()
  public async claimOneTimeKeys(
    userDeviceMap: Record<string, Record<string, OTKAlgorithm>>,
    federationTimeoutMs = 10000,
  ...
  public async sendToDevices(
    ...
    return this.doRequest(
      "PUT",
      `/_matrix/client/v3/sendToDevice/${encodeURIComponent(type)}/${encodeURIComponent(txnId)}`,
      null,
      {
        messages: messages,
      },
   ...
   */
  @requiresCrypto()
  public async signAndCreateKeyBackupVersion(
    info: IKeyBackupInfoUnsigned,
   ...
   */
  @requiresCrypto()
  public updateKeyBackupVersion(
    version: KeyBackupVersion,
    info: IKeyBackupInfoUpdate,
  ): Promise<void> {
    const data = {
      ...info,
   ...
   */
  @timedMatrixClientFunctionCall()
  public async getRelationsForEvent(
    roomId: string,
    eventId: string,
    relationType?: string,
    eventType?: string,
  ...
  public doRequest(
    ...
    if (this.impersonatedDeviceId) {
      if (!qs) qs = { device_id: this.impersonatedDeviceId };
      else qs["device_id"] = this.impersonatedDeviceId;
    }
    const headers = {};
    if (this.accessToken) {
