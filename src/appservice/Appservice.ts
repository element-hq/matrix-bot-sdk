...
import { checkPrimeSync, randomUUID } from "crypto";
...
export class Appservice extends EventEmitter {
  ...
  constructor(private options: IAppserviceOptions) {
    ...
    options.joinStrategy = new AppserviceJoinRoomStrategy(
    ...
    this.app.get(
      ...
      this.onThirdpartyProtocol.bind(this),
    );
    this.app.get(
      "/_matrix/app/v1/thirdparty/user/:protocol",
      this.onThirdpartyUser.bind(this),
    );
    this.app.get(
    ...
    this.app.get(
      ...
      this.onThirdpartyLocation.bind(this),
    );
    this.app.get(
    ...
    this.app.post(
      ...
      this.onKeysClaim.bind(this),
    );
    this.app.post(
      "/_matrix/app/unstable/org.matrix.msc3984/keys/query",
      this.onKeysQuery.bind(this),
    );
    ...
    function getPrefix(
      namespace: { regex: string }[] | undefined,
    ): string | null {
      const prefix =
        namespace?.length === 1 && (namespace[0].regex || "").split(":")[0];
      if (!prefix) {
        return null;
      }
    ...
    this.userPrefix =
      options.userPrefix !== undefined
        ? options.userPrefix
        : getPrefix(this.registration.namespaces.users);
    this.aliasPrefix =
      options.aliasPrefix !== undefined
        ? options.aliasPrefix
        : getPrefix(this.registration.namespaces.aliases);
    this.apis = new AppserviceApis(
      new MatrixClient(options.homeserverUrl, this.registration.as_token),
  ...
  public getUserIdForSuffix(suffix: string): string {
    if (!this.userPrefix) {
      throw new Error(
        `Cannot use getUserIdForSuffix, provided user namespace did not contain exactly one valid namespace`,
      );
    }
    ...
    if (!intent) {
      ...
      if (this.options.intentOptions.encryption) {
        intent.enableEncryption().catch((e) => {
          LogService.error(
            "Appservice",
            `Failed to set up crypto on intent ${userId}`,
            e,
          );
  ...
  public getSuffixForUserId(userId: string): string {
    ...
    if (
      !userId ||
      !userId.startsWith(this.userPrefix) ||
      !userId.endsWith(`:${this.options.homeserverName}`)
    ) {
      // Invalid ID
      return null;
    }
  ...
  public setRoomDirectoryVisibility(
    ...
    return this.botClient.doRequest(
      "PUT",
  ...
  private async decryptAppserviceEvent(
    roomId: string,
    encrypted: EncryptedRoomEvent,
  ): ReturnType<Appservice["processEvent"]> {
    ...
    const decryptFn = async (client: MatrixClient) => {
      ...
      LogService.info(
        "Appservice",
        `Processing decrypted event of type ${event["type"]}`,
      );
      return event;
    };
    // 1. Try cached client
    if (existingClient) {
      try {
        ...
        LogService.debug(
          "Appservice",
          `Failed to decrypt via cached client ${await existingClient.getUserId()}`,
          error,
        );
        LogService.warn(
          "Appservice",
          `Cached client was not able to decrypt ${roomId} ${encrypted.eventId} - trying other intents`,
        );
      }
    }
    ...
    if (this.botClient.crypto?.isReady) {
      try {
        ...
        LogService.debug(
          "Appservice",
          `Failed to decrypt via bot client`,
          error,
        );
        LogService.warn(
          "Appservice",
          `Bot client was not able to decrypt ${roomId} ${encrypted.eventId} - trying other intents`,
        );
      }
    }
    ...
    ).filter((u) => this.isNamespacedUser(u));
    ...
    for (const intentCacheEntry of this.intentsCache.entries()) {
      ...
      try {
        ...
        LogService.debug(
          "Appservice",
          `Failed to decrypt via ${userId}`,
          error,
        );
        LogService.warn(
          "Appservice",
          `Existing encrypted client was not able to decrypt ${roomId} ${encrypted.eventId} - trying other intents`,
        );
      }
    }
    ...
    const userInRoom = this.intentsCache.find(
      (intent, userId) =>
        userIdsInRoom.includes(userId) &&
        intent.underlyingClient.crypto?.isReady,
    );
    if (!userInRoom) {
    ...
    try {
      ...
      LogService.debug(
        "Appservice",
        `Failed to decrypt via random user ${userInRoom.userId}`,
        error,
      );
  ...
  private async handleTransaction(
    ...
    if (Array.isArray(body["de.sorunome.msc2409.to_device"])) {
      orderedEdus.push(
        ...body["de.sorunome.msc2409.to_device"].map((e) => ({
          ...e,
          unsigned: {
            ...e["unsigned"],
    ...
    if (Array.isArray(body["de.sorunome.msc2409.ephemeral"])) {
      orderedEdus.push(
        ...body["de.sorunome.msc2409.ephemeral"].map((e) => ({
          ...e,
          unsigned: {
            ...e["unsigned"],
    ...
    for (let event of orderedEdus) {
      ...
      LogService.info(
        "Appservice",
        `Processing ${event["unsigned"][EDU_ANNOTATION_KEY]} event of type ${event["type"]}`,
      );
      ...
      if (
        ...
        (event["type"] === "m.room.encrypted" ||
          event.unsigned?.[EDU_ANNOTATION_KEY] === EduAnnotation.ToDevice)
      ) {
    ...
    const deviceLists = (body["org.matrix.msc3202.device_lists"] as {
      changed: string[];
      removed: string[];
    }) ?? {
      changed: [],
    ...
    if (otks && this.cryptoStorage) {
      for (const userId of Object.keys(otks)) {
        ...
        await intent.enableEncryption();
        const otksForUser =
          otks[userId][intent.underlyingClient.crypto.clientDeviceId];
        if (otksForUser) {
          if (!byUserId[userId]) {
            byUserId[userId] = {
              counts: null,
    ...
    if (fallbacks && this.cryptoStorage) {
      for (const userId of Object.keys(fallbacks)) {
        ...
        const fallbacksForUser =
          fallbacks[userId][intent.underlyingClient.crypto.clientDeviceId];
        if (
          Array.isArray(fallbacksForUser) &&
          !fallbacksForUser.includes(OTKAlgorithm.Signed)
        ) {
          if (!byUserId[userId]) {
            byUserId[userId] = {
              counts: null,
    ...
    if (this.cryptoStorage) {
      for (const userId of Object.keys(byUserId)) {
        ...
        if (!info.unusedFallbacks)
          info.unusedFallbacks = JSON.parse(
            (await userStorage.readValue("last_unused_fallbacks")) || "[]",
          );
        if (!info.counts)
          info.counts = JSON.parse(
            (await userStorage.readValue("last_counts")) || "{}",
          );
    ...
    for (let event of body.events as any[]) {
      ...
      if (event["type"] === "m.room.encrypted") {
        ...
        if (this.cryptoStorage) {
          ...
          try {
            ...
            LogService.error(
              "Appservice",
              `Decryption error on ${event["room_id"]} ${event["event_id"]}`,
              e,
            );
            ...
            try {
              ...
              LogService.error(
                "Appservice",
                `Still failed ${event["room_id"]} ${event["event_id"]}`,
                e,
              );
            }
          }
      ...
      if (
        ...
        this.isNamespacedUser(event["state_key"])
      ) {
  ...
  private async onTransaction(
    ...
    try {
      ...
      try {
        ...
        LogService.warn(
          "Appservice",
          "Failed to store completed transaction",
          ex,
        );
      }
      res.status(200).json({});
  ...
  private async onUser(
    ...
    this.emit("query.user", userId, async (result) => {
      ...
      if (result === false) {
        res
          .status(404)
          .json({ errcode: "USER_DOES_NOT_EXIST", error: "User not created" });
      } else {
        const intent = this.getIntentForUserId(userId);
  ...
  private async onRoomAlias(
    ...
    this.emit("query.room", roomAlias, async (result) => {
      ...
      if (result === false) {
        res
          .status(404)
          .json({ errcode: "ROOM_DOES_NOT_EXIST", error: "Room not created" });
      } else {
  ...
  private async onKeysClaim(
    ...
    this.emit(
      ...
      (
        result:
          | MSC3983KeyClaimResponse
          | Promise<MSC3983KeyClaimResponse>
          | undefined
          | null,
      ) => {
        responded = true;
        ...
        Promise.resolve(result)
          ...
          .catch((e) => {
            ...
            res.status(500).json({
              errcode: "M_UNKNOWN",
              error: "Error handling key claim API",
            });
          });
      },
    );
    if (!responded) {
      res
        .status(404)
        .json({ errcode: "M_UNRECOGNIZED", error: "Endpoint not implemented" });
    }
  }

  private async onKeysQuery(
    req: express.Request,
    res: express.Response,
  ): Promise<any> {
    if (!this.isAuthed(req)) {
    ...
    this.emit(
      ...
      (
        result:
          | MSC3984KeyQueryResponse
          | Promise<MSC3984KeyQueryResponse>
          | undefined
          | null,
      ) => {
        responded = true;
        ...
        Promise.resolve(result)
          ...
          .catch((e) => {
            ...
            res.status(500).json({
              errcode: "M_UNKNOWN",
              error: "Error handling key query API",
            });
          });
      },
    );
  ...
  private onThirdpartyProtocol(req: express.Request, res: express.Response) {
    ...
    this.emit(
      "thirdparty.protocol",
      protocol,
      (protocolResponse: IApplicationServiceProtocol) => {
  ...
  private handleThirdpartyObject(
    ...
    if (protocol) {
      // If protocol is given, we are looking up a objects based on fields
      if (!this.registration.protocols.includes(protocol)) {
        res.status(404).json({
          errcode: "PROTOCOL_NOT_HANDLED",
          error: "Protocol is not handled by this appservice",
      ...
      // If a user ID is given, we are looking up a remote objects based on a id
  ...
  private onThirdpartyUser(req: express.Request, res: express.Response) {
    return this.handleThirdpartyObject(
      req,
      res,
      "user",
  ...
  private onPing(req: express.Request, res: express.Response) {
    ...
    if (!this.pingRequest) {
      res
        .status(400)
        .json({ errcode: "BAD_REQUEST", error: "No ping request expected" });
      return;
    }
    if (req.body.transaction_id !== this.pingRequest) {
      res.status(400).json({
        errcode: "BAD_REQUEST",
        error: "transaction_id did not match",
      });
      return;
    }
